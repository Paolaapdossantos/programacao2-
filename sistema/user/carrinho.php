...
if (isset($_GET["a"])) {
	...
	header("Location:/programacao2-/sistema/user/carrinho.php");
	exit;
} elseif (isset($_GET["r"])) {
	if (isset($_COOKIE["carrinho"])) {
...
if (isset($_COOKIE["carrinho"])) {
	...
	if ($result) {
		while ($row = mysqli_fetch_assoc($result)) {
			echo "<a href=\"/programacao2-/sistema/user/carrinho.php?r=".$row["id"]."\" style=\"color: black;\">(-)</a> ".$row["nome"]."<br>";
		}
	}
} else {
...
<br><button><a href="/programacao2-/sistema/user/index.php" style="color: black;">Voltar</a></button>
